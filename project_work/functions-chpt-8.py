...
def make_pizza(size, *toppings):
    """Summarizethe pizza we are about to make."""
...
# pretend this is a separate file
import pizza
...
# You don’t actually see code being copied between files because Python copies the code behind the scenes, just before the program runs. All you need
# to know is that any function defined in pizza.py will now be available in
# making_pizzas.py.
# To call a function from an imported module, enter the name of the module you imported, pizza, followed by the name of the function, make
# _pizza(), separated by a dot. This code produces the same output as the original program that didn’t import a module
...
# only more concise than pizza.make_pizza(), but it also redirects your attention from the module name and 
# allows you to focus on the descriptive names of its functions. These function names, which clearly tell you what each function does, are more important to the readability of your code than using the full module name.
...
# if the module has a function name that matches an existing name in your project, 
# you can get unexpected results. Python may see several functions or variables with the same name, 
...
def function_name(parameter_0, paremter_1='default value')
